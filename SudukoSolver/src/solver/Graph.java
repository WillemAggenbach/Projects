 ...
 * through <em>V</em> – 1. It supports the following two primary operations:
 * add an edge to the graph, iterate over all of the vertices adjacent to a
 * vertex. It also provides methods for returning the number of vertices
 * <em>V</em> and the number of edges <em>E</em>. Parallel edges and self-loops
 * are permitted. By convention, a self-loop <em>v</em>-<em>v</em> appears in
 * the adjacency list of <em>v</em> twice and contributes two to the degree of
 * <em>v</em>.
 * <p>
...
public class Graph {
	...
	// this.V = in.readInt();
	// if (V < 0) {
	// throw new IllegalArgumentException("number of vertices in a Graph must be
	// nonnegative");
	// }
	// adj = new Bag[V];
	// for (int v = 0; v < V; v++) {
	...
	// int E = in.readInt();
	// if (E < 0) {
	// throw new IllegalArgumentException("number of edges in a Graph must be
	// nonnegative");
	// }
	// for (int i = 0; i < E; i++) {
	// int v = in.readInt();
